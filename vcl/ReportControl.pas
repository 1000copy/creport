  ...
  Begin
    ...
    Begin
      ...
      If (UpperCase(ThisCell.FCellText) = '`PAGENUM') Then //李泽伦增
        thiscell.FCellText := '第 ' + inttostr(PageNumber) + ' 页'
      Else If (UpperCase(ThisCell.FCellText) = '`PAGENUM/') Then //李泽伦增
      begin
        thiscell.FCellText := '第 ' + inttostr(PageNumber) + '/' +
          inttostr(FPageAll) + ' 页';
      MessageDlg(thiscell.FCellText, mtInformation,[mbOk], 0);

      end
      Else If (UpperCase(ThisCell.FCellText) = '`PAGENUM-') Then //李泽伦增
        thiscell.FCellText := '第 ' + inttostr(Fpageall) + '-' +
          inttostr(PageNumber) + ' 页'
      Else If copy(UpperCase(ThisCell.FCellText), 1, 9) = '`SUMPAGE(' Then  //李泽伦增
        thiscell.FCellText := setSumpageYg(thiscell.FCellDispformat,
          ThisCell.FCellText)
      Else If copy(UpperCase(ThisCell.FCellText), 1, 8) = '`SUMALL(' Then  //李泽伦增
        thiscell.FCellText := setSumAllYg(thiscell.FCellDispformat,ThisCell.FCellText);
...
Procedure TReportRunTime.PreparePrint; // 在本控件中无用,已被 PreparePrintk代替,李泽伦
Var
  I, J, K, L, M, N, TableCount: Integer;
  ThisLine, TempLine: TReportLine;
  ThisCell, NewCell, TempOwnerCell: TReportCell;
  MainDataset, TempDataSet: TDataset;
  TempField: TField;
  TempCellTable: TCellTable;
  TempLineCount: Integer;
  TempDataSetList: TList;
  bHasDataset, bBreak: Boolean;
  nHeightRemain: Integer;
  nContinued: Integer;
  PrevM, PrevJ, TempM: Integer;
  nLineCount: Integer;
  nCurTable: Integer;
  NextPageLine: TReportLine;
Begin
  PrevJ := 0;
  // 取得主表的数据集
  If FPrintLineList = Nil Then
    FPrintLineList := TList.Create;
  If fallprint Then                     // 全部打印，1999.1.19 廖伯志
  Begin
    For I := 0 To Cp_DFdList.Count - 1 Do
    Begin
      TDataSetItem(Cp_DFdList[I]).pDataSet.First;
    End;
  End;
  ...
  //扫描模板中的所有格子，确定是否有关联数据表的主数据表,同时确定求和数据字段
  For I := 0 To FLineList.Count - 1 Do
  Begin
    ...
    Begin
      ...
      ThisCell := TReportCell(ThisLine.FCells[J]);
      If Length(ThisCell.CellText) > 0 Then
      Begin
        If ThisCell.CellText[1] = '@' Then
        Begin
          MainDataset := GetDataset(ThisCell.CellText);
        End;
  ...
  Else
  Begin
    If fallprint Then                   //     全部打印
    Begin
      TableCount := MainDataset.RecordCount;
      // MainDataset.DisableControls;
      //没有加入DisableControls ，留给用户控制是否DISABLECONTROLS 1999.03.05 wang han song
      MainDataset.First;
    End
    Else
      tablecount := 1;                  //打印当前记录
  ...
  Begin
    ...
    nHeightRemain := FHeaderHeight;

    // 在NewTable时检查纸张剩下的空间是否够表头+表尾，若不够则换页
    If FPrintLineList.Count > 0 Then
    Begin
      If TReportLine(FPrintLineList.Last).LineRect.Bottom > Height -
        FBottomMargin - FHeaderHeight Then
      Begin
        If FPrintLineList.Count > 0 Then
          SaveTempFile(FPageCount, 1);
      ...
      End;
    End;
    While J < FLineList.Count Do
    Begin
      bHasDataSet := False;
      // 每一行数据的处理
      ThisLine := TReportLine(FLineList[J]);
      // 根据每一个CELL的数据集来计算需要几行，创建这些行
      TempLineCount := 1;
      For M := 0 To ThisLine.FCells.Count - 1 Do
      Begin
        ThisCell := TReportCell(ThisLine.FCells[M]);
        If Length(ThisCell.CellText) > 0 Then
        Begin
          If ThisCell.CellText[1] = '#' Then
          Begin
            ...
            Begin
              ...
              If TempDataSet.RecordCount > TempLineCount Then
                TempLineCount := GetDataSet(ThisCell.CellText).RecordCount;
              //创建子表行
            End;
          End;
        End;
      End;
      // 绘制表尾时
      If (nContinued = 1) And bHasDataSet Then
      Begin
        Inc(J);
        Continue;
      End;
      // 表头绘制完毕
      If (nContinued = 2) And (PrevJ = J) Then
      Begin
        //TempM := PrevM;
        nContinued := 0;
      End
      ...
      // 绘制表头时
      If (nContinued = 2) And bHasDataset And (J < PrevJ) Then
      Begin
        Inc(J);
        Continue;
      End;
      {
            // 郭家骏 add @ 1998.10.27
            if (TempLineCount = 1) and (I > 0) and (I < TableCount - 1) then
            begin
              J := J + 1;
              continue;
            end;
      ...
      For M := TempM To TempLineCount - 1 Do
      Begin
        // 记录带有数据集的行数
        If bHasDataSet Then
        ...
        For K := 0 To ThisLine.FCells.Count - 1 Do
        Begin
          ThisCell := TReportCell(ThisLine.FCells[K]);
          NewCell := TReportCell.Create;
          TempLine.FCells.Add(NewCell);
          NewCell.FOwnerLine := TempLine;
          ...
          Begin
            ...
            // string
            If (Length(ThisCell.CellText) > 0) And (ThisCell.OwnerCell = Nil)
              Then
            Begin
              If (ThisCell.FCellText[1] = '`') Then
              Begin
                If (UpperCase(ThisCell.FCellText) = '`PAGENUM') And (nCurTable >
                  1) Then
                  FCellText := ''
                Else
                  FCellText := GetVarValue(ThisCell.FCellText);
              End
            ...
            Begin
              ...
              For L := 0 To FOwnerCellList.Count - 1 Do
              Begin
                If ThisCell.OwnerCell = TCellTable(FOwnerCellList[L]).PrevCell
                  Then
                  TempOwnerCell :=
                    TReportCell(TCellTable(FOwnerCellList[L]).ThisCell);
              End;
        ...
        If Not bHasDataSet Then
          nHeightRemain := nHeightRemain -
            TReportLine(FPrintLineList.Last).LineHeight;

        If bHasDataSet And (((TReportLine(FPrintLineList[FPrintLineList.Count -
          1]).LineRect.Bottom > Height - FBottomMargin - nHeightRemain) And
            FNewTable) Or
          (nLineCount >= FDataLine)) Then // and (M <> TempLineCount - 1) then
        Begin
          // 判断是否是否是最后一个数据源的最后一条记录时正好打满一个表
          // 即查找是否后面还有数据源

          If M >= TempLineCount - 1 Then
          Begin
            bBreak := True;
            For K := J + 1 To FLineList.Count - 1 Do
            Begin
              For N := 0 To TReportLine(FLineList[K]).FCells.Count - 1 Do
              Begin
                If
                  (Length(TReportCell(TReportLine(FLineList[K]).FCells[N]).FCellText) > 0) Then
                Begin
                  If
                    (TReportCell(TReportLine(FLineList[K]).FCells[N]).FCellText[1]
                    = '#') Or
                    (TReportCell(TReportLine(FLineList[K]).FCells[N]).FCellText[1] = '@') Then
                    bBreak := False;
                End;
              End;
            End;
          ...
          nHeightRemain := nHeightRemain + FHeaderHeight;

          TempDataSetList := TList.Create;

          For K := 0 To ThisLine.FCells.Count - 1 Do
          Begin
            ThisCell := TReportCell(ThisLine.FCells[K]);
            If Length(ThisCell.FCellText) > 0 Then
            Begin
              If ThisCell.CellText[1] = '#' Then
              Begin
                TempDataSet := GetDataSet(ThisCell.CellText);
                If (TempDataSet <> Nil) And (TempDataSetList.IndexOf(TempDataSet)
                  < 0) Then
          ...
          If fallprint Then
          Begin
            For K := 0 To TempDataSetList.Count - 1 Do
            Begin
              If Not TDataSet(TempDataSetList[K]).Eof Then
                TDataSet(TempDataSetList[K]).Next;
            End;
          End
          Else
            break;
          TempDataSetList.Free;
          break;
        End
        Else If TReportLine(FPrintLineList[FPrintLineList.Count -
          1]).LineRect.Bottom > Height - FBottomMargin - nHeightRemain Then
        Begin
          If FPrintLineList.Count > 0 Then
            //  SaveTempFile(FPageCount);
      ...
      If (nContinued = 1) And (J = FLineList.Count - 1) Then
      Begin
        // 绘制新表头状态
        nContinued := 2;
        J := 0;
        nCurTable := nCurTable + 1;
        If nCurTable > FTablePerPage Then
        Begin
          nCurTable := 1;
          If FPrintLineList.Count > 0 Then
            // SaveTempFile(FPageCount);
          ...
          // clear the temp data here
          For K := FPrintLineList.Count - 1 Downto 0 Do
          Begin
            ThisLine := TReportLine(FPrintLineList[K]);
            ThisLine.Free;
          End;

          FPrintLineList.Clear;

          For K := FOwnerCellList.Count - 1 Downto 0 Do
            TCellTable(FOwnerCellList[K]).Free;
 ...
 // For I := 0 To FPrintLineList.Count - 1 Do
  //  Begin
  //    ThisLine := TReportLine(FPrintLineList[I]);
      //nHeightRemain := nHeightRemain + ThisLine.LineHeight;
 //   End;

  If fallprint Then
  Begin
    //    if (FPrintLineList.Count > 0) and (nHeightRemain > FHeaderHeight) then
   // SaveTempFile(FPageCount);
    //    if (nHeightRemain > 0) and (nHeightRemain <= FHeaderHeight) then
    //      FPageCount := FPageCount - 1;
...
{procedure TReportRunTime.UpdatePrintLinesk(xxx:Tlist);
var
  PrevRect, TempRect: TRect;
  I, J: Integer;
  ThisLine: TReportLine;
...
begin
  ...
  begin
    ...
    if I = 0 then
      ThisLine.LineTop := FTopMargin;
    if I > 0 then
      ThisLine.LineTop := TReportLine(xxx[I - 1]).LineTop + TReportLine(xxx[I - 1]).LineHeight;
...
{procedure TDatasetToExcel.SetFileName(const value: TFileName);
begin
 FFileName:=value;
end;
}
